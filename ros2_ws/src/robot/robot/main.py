...
PURE_PURSUIT_CONTROL_POINTS = [
    ...
    (620.0, 3700.0),  # Start turn early
    (690.0, 3540.0),  # Mid-corner apex
    (730.0, 3300.0),  # Easing out of turn
    (750.0, 3000.0),  # Fully straightened out safely
    (950.0, 600.0),   # Safe line down center of ramp
    (1600.0, 600.0),
]
...
# --- MISSION PIPELINE CONFIGURATION ---
...
def run(robot: Robot) -> None:
    print("[INIT] Configuring sensors and odometry allocations...")
    ...
    while True:
        if state == "INIT":
            ...
            # Direct press start to run the sequence (ignoring vision traffic checks)
            if robot.was_button_pressed(Button.BTN_5): 
                print("[FSM] Starting mission pipeline.")
                state = "BURGER_PICKUP"
            time.sleep(0.1)
            ...
            print("[FSM] Burger pickup complete. Passing navigation loop execution to pipeline...")
            
            # Synchronize timing parameters for pipeline loops
            course_stage_index = 0
            ...
            # Security button 2 cancellation override
            if robot.was_button_pressed(Button.BTN_2):
                if motion_handle is not None:
                    motion_handle.cancel()
                    motion_handle.wait(timeout=1.0)
                robot.stop()
                print("[FSM] IDLE — course trajectory aborted via button command.")
                state = "DONE"
                
            else:
                # Periodic terminal trace outputs
                if now - last_status_print_at >= STATUS_PRINT_INTERVAL_S:
                    print_course_status(robot, course_stage_index)
                    last_status_print_at = now
                
                # Check for stage termination and route seamlessly into the next
