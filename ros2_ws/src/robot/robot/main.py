...
def run(robot: Robot) -> None:
    ...
    while True:
        ...
        # ---------------------------------------------------------
        # 2. BURGER PICKUP SEQUENCE (FULLY RESTORED)
        # ---------------------------------------------------------
            ...
            if time.monotonic() - action_timer > 0.6:
        ...
        # --- Sub-Routines for Arm Actions (RESTORED ACCURATE DELAYS) ---
        elif state == "DO_PICK":
            ...
            if action_sub_state == "LIFT_DOWN_TO_STACK":
                ...
                if abs(current - LIFT_PICKUP_TICKS) <= LIFT_TOLERANCE or (time.monotonic() - action_timer > LIFT_TIMEOUT_S):
                    requested_claw_deg = CLAW_CLOSE_BUN_DEG
                    action_timer = time.monotonic()
                    action_sub_state = "WAIT_CLOSE_FINAL"

            elif action_sub_state == "WAIT_CLOSE_FINAL":
                if time.monotonic() - action_timer > 0.6:
                    requested_final_ticks = LIFT_CARRY_TICKS
                    action_timer = time.monotonic()
                    action_sub_state = "WAIT_LIFT_UP"
        ...
        # --- Physical Execution of Claw Movement ---
        robot.enable_servo(CLAW_SERVO)
        robot.set_servo(CLAW_SERVO, requested_claw_deg)
