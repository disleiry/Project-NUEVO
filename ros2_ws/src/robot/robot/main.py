...
_ESTOP_IMMUNE = frozenset({"INIT", "INIT_JOG", "INITIAL_MOVE_FORWARD", "TURN_TO_LIGHT", "WAIT_GREEN_LIGHT", "RETURN_TO_HEADING"})
...
def run(robot: Robot) -> None:
    ...
    while True:
        ...
        if state == "INIT":
            ...
            robot.move_forward(distance=200.0, velocity=DRIVE_VELOCITY, tolerance=POS_TOLERANCE_MM, blocking=True)
            robot.stop()
            state = "TURN_TO_LIGHT"

        elif state == "TURN_TO_LIGHT":
            robot.turn_by(delta_deg=45.0, blocking=True, tolerance_deg=TURN_TOLERANCE_DEG)
            robot.stop()
        ...
        if abs(current_phys - requested_final_ticks) > LIFT_TOLERANCE:
            ...
            if now - last_step_time > step_delay_s:
                ...
                robot.set_motor_position(
                    LIFT_MOTOR, requested_final_ticks,
