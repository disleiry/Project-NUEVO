...
TRAFFIC_LIGHT_LOOK_ANGLE_DEG = 135.0  
...
def print_course_status(robot: Robot, stage_index: int) -> None:
    ...
    if obstacle_tracks:
        nearest_boundary_mm = min(
            max(
                0.0,
                ((float(track["x"]) - x) ** 2 + (float(track["y"]) - y) ** 2) ** 0.5 - float(track["radius"]),
            )
...
def print_config(robot: Robot) -> None:
    ...
    print(f"[CFG] LAPF velocity={LAPF_VELOCITY_MM_S:.0f} mm/s tolerance={LAPF_TOLERANCE_MM:.0f} mm")
    print(
        f"[CFG] theta={LIDAR_MOUNT_THETA_DEG:.1f}° filter={LIDAR_RANGE_MIN_MM:.0f}-"
    ...
    while True:
        ...
        # ── TURN_TO_LIGHT ───────────────────────────────────────────────────
            ...
            if robot.was_button_pressed(Button.BTN_2):
                ...
                if motion_handle is not None and motion_handle.is_finished():
                    robot.stop()
                    motion_handle = None
                    print("[FSM] WAIT_FOR_GREEN — absolute traffic-light angle reached")
        ...
        # ── WAIT_FOR_GREEN ──────────────────────────────────────────────────
            ...
            if robot.was_button_pressed(Button.BTN_2):
                ...
                if traffic_light_color == "green":
                    ...
                    # No detection yet: remain stopped at the viewing angle.
                    pass
        ...
        # ── RETURN_TO_FORWARD ───────────────────────────────────────────────
            ...
            if robot.was_button_pressed(Button.BTN_2):
                ...
                motion_handle = None
                show_idle_leds(robot)
                print("[FSM] IDLE — mission cancelled while returning to forward heading")
                state = "IDLE"
            else:
                if now - last_status_print_at >= STATUS_PRINT_INTERVAL_S:
                    _, _, theta = robot.get_odometry_pose()
                    print(f"  turning back to forward heading: θ={theta:.1f}° target={TRAFFIC_LIGHT_RETURN_ANGLE_DEG:.1f}°")
                    last_status_print_at = now

                if motion_handle is not None and motion_handle.is_finished():
                    robot.stop()
                    motion_handle = None
                    print("[FSM] COURSE_MOVING — forward heading reached, starting pure pursuit")
                    course_stage_index = 0
        ...
        # ── COURSE_MOVING ───────────────────────────────────────────────────
            ...
            if robot.was_button_pressed(Button.BTN_2):
                ...
                robot.set_led(LED.GREEN, 0)
                print("[VISION] stop sign detected during course — STOPPING")
                state = "IDLE"
            else:
                if now - last_status_print_at >= STATUS_PRINT_INTERVAL_S:
                ...
                if motion_handle is not None and motion_handle.is_finished():
                    ...
                    if course_stage_index >= len(MISSION_STAGES):
                        ...
                        motion_handle = None
                        show_idle_leds(robot)
                        print("[FSM] IDLE — mission complete!")
                        state = "IDLE"
                    else:
                        if STAGE_PAUSE_S > 0.0:
                            robot.stop()
                            print(f"[FSM] Pausing for {STAGE_PAUSE_S:.1f}s between stages...")
