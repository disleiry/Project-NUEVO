...
2. Turn to face the traffic light, wait until it is green, then turn forward.
    ...
    while True:
        ...
        if state == "INIT":
            ...
            if robot.was_button_pressed(Button.BTN_1):
                ...
                internal_target_ticks = 0
                time.sleep(0.15)
                print("[INIT] Encoder zeroed -> TURN_TO_LIGHT")
                dim_all_leds(robot)
                led_moving(robot)

                print(
                    f"[FSM] TURN_TO_LIGHT — turning to absolute heading "
